...
@tool(name="load_rooms", description="Load all rooms from the database.")
def load_rooms(file_path: Path = ROOMS_FILE) -> List[Room]:
    with open(file_path, 'r') as f:
        return [Room(**room) for room in json.load(f)]

@tool(name="find_matching_rooms", description="Find rooms that match the required capacity and equipment.")
...
@tool(name="find_similar_rooms", description="Find rooms with similar equipment and capacity.")
...
@tool(name="find_rooms_by_equipments", description="Find rooms that have all the specified equipment.")
